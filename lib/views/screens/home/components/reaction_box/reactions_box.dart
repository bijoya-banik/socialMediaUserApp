...
import 'package:buddyscripts/models/reaction_button/drag_data_model.dart';
...
import 'package:buddyscripts/views/screens/home/components/reaction_box/reactions_box_item.dart';
import 'package:buddyscripts/views/screens/home/components/reaction_box/widget_size_render_object.dart';
...
class _ReactionsBoxState extends State<ReactionsBox> with TickerProviderStateMixin {
        ...
        children: [
          Positioned.fill(
            child: Listener(
              ...
              child: Container(color: KColor.transparent),
            ),
          ),
          PositionedDirectional(
            top: top - (widget.isFromComments ? 45 : 5),
            start: 20,
    ...
    return WidgetSizeOffsetWrapper(
      onSizeChange: (Size size) {
        ...
        if (_boxSizeController.isCompleted) {
          ...
          AudioPlayerService.playSound('icon_focus.mp3');
      ...
      child: Padding(
        ...
        child: GestureDetector(
          onTapDown: (point) {
            _dragData = DragData(offset: point.globalPosition);
  ...
  //   if (buttonX + (_boxSizeAnimation.value?.width ?? 0) < screenWidth) return buttonX - buttonRadius;
  ...
  double _getVerticalPosition() {
    ...
    if (_getTopPosition() - widget.buttonSize.height * 4.5 < 0) {
      // print(' TOP space not enough for the box');
    ...
    if (_getBottomPosition() + (widget.buttonSize.height * 5.5) > context.screenSize.height) {
      // print(' BOTTOM space not enough for the box');
      return _getTopPosition();
    }

    if (widget.position == Position.TOP) {
      // print('widget.position == Position.TOP');
      return _getTopPosition();
    }
  ...
  double _getTopPosition() {
    // print('_getTopPosition');
    // print(widget.buttonOffset.dy + (widget.buttonSize.height * 5));
    return widget.buttonOffset.dy - (widget.buttonSize.height * 2.65);
  }

  double _getBottomPosition() {
    // print('_getBottomPosition');
    // print(widget.buttonOffset.dy);
